...
export function registerOrganizationRoutes(app: FastifyInstance, ctx: AppContext): void {
  ...
  // Clerk admin principal (any authenticated human), not admin-of-a-
  ...
  app.post<{ Body: { name: string } }>("/v1/organizations", async (request, reply) => {
    ...
    if (ctx.clerkConfig) {
      const admin = await resolveAdminPrincipal(request.headers.authorization, ctx.clerkConfig);
      if (!admin) {
        return reply.code(401).send({ error: "missing or unrecognized admin session token" });
      }
      const org = ctx.repo.createOrganization(name);
      ctx.repo.createOrgAdmin(org.id, admin.clerkUserId);
  ...
  app.get("/v1/organizations", async (request, reply) => {
    if (!ctx.clerkConfig) {
      return reply.code(400).send({
        error: "GET /v1/organizations requires admin auth to be configured (CLERK_SECRET_KEY unset)",
      });
    }
    const admin = await resolveAdminPrincipal(request.headers.authorization, ctx.clerkConfig);
    if (!admin) {
    ...
    return reply.send({ organizations: ctx.repo.listOrganizationsForAdmin(admin.clerkUserId) });
  });
}
